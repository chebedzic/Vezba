...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<Photo> GetMainPhotoForUser(int idUser)
        {
            return await _context.Photos.FirstOrDefaultAsync(p=>p.UserId == idUser && p.IsMain);
        }
        ...
        public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recepientId)
        {
            var messages = await _context.Messages
                ...
                .Where(m => m.RecipientId == userId && m.RecipientDeleted == false && m.SenderId == recepientId || m.RecipientId == recepientId && m.SenderDeleted == false && m.SenderId == userId)
